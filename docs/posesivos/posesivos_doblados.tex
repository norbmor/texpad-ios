...
\begin{center}
\begin{tabular}{c}
$\beta : \sigma^a$ \quad---\quad el argumento pasa arriba \textbf{intacto}\\[4pt]
$\bullet$\\[4pt]
$\alpha(\beta) : \tau^c$ \quad---\quad la IC se almacena \textbf{aparte}\\
\end{tabular}
\end{center}

El punto crucial es que el nodo madre hereda $\beta$, no $\alpha(\beta)$: el expresivo no modifica en absoluto el significado veritativo-condicional, solo deposita una proposición en una dimensión paralela. De ahí que Saab hable de \textit{pure expressives}.
